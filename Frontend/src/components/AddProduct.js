  ...
  const addProduct = () => {
    ...
    fetch(`${process.env.REACT_APP_BACKEND_URL}api/product/add`, {
      ...
      .then((result) => {
        ...
        notificationContext.addNewProductNotification(product.name);

        // Check if stock is 0 and add low stock notification
        notificationContext.addLowStockNotification(product.name, 0);
